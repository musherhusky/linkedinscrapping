...
export async function savePost(userId, post, status = 'sent', hallonResponse = null, sourceType = 'company') {
  const supabase = getSupabaseClient();
  ...
  try {
    const { error } = await supabase
      ...
      .insert({
        ...
        external_id: hallonResponse?.id || null,
        dispatch_response: hallonResponse,
        status: status,

...
export async function getAllUsersForHour(_hourUtc) {
  const supabase = getSupabaseClient();

  logger.debug(`Obteniendo todos los usuarios activos (cron diario)`);
  ...
  try {
    ...
    if (!activeUsers || activeUsers.length === 0) {
      logger.warn(`No hay usuarios con auto_execution_enabled — posible configuración vacía`);
      return [];
    }

    logger.success(`${activeUsers.length} usuario(s) activo(s)`);

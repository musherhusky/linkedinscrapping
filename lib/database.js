...
export async function getAllUsersForHour(hourUtc) {
  ...
  try {
    ...
    // Filtrar solo usuarios con auto_execution_enabled
...
export async function getActiveTerms(userId, supabase = getSupabaseClient()) {
  logger.debug(`Obteniendo términos activos del usuario: ${userId}`);

  try {
    const { data, error } = await supabase
      .from('target_search_terms')
      .select('term')
      .eq('user_id', userId)
      .eq('active', true);
    ...
    logger.success(`${terms.length} término(s) activo(s) encontrado(s)`);

    return terms;
  } catch (error) {
    logger.error(`Error obteniendo términos: ${error.message}`);
    throw error;
  }
}
  ...
  try {
    const { error } = await supabase
      ...
      .insert({
        ...
        search_term:        sourceType === 'term' ? (post.queryTargetUrl || null) : null,
    ...
    logger.error(`Error guardando post (userId: ${userId}, url: ${post.url}): ${error.message}`);
    throw error;
  }
}
...
export async function saveApiUsage(userId, provider, stats, supabase = getSupabaseClient()) {
  try {
    const { error } = await supabase
      .from('api_usage_logs')
      .insert({
        user_id:            userId,
        provider:           provider,
        model_or_actor:     stats.modelOrActor || null,
        input_tokens:       stats.inputTokens ?? null,
        output_tokens:      stats.outputTokens ?? null,
        compute_units:      stats.computeUnits ?? null,
        posts_received:     stats.postsReceived || 0,
        estimated_cost_usd: stats.estimatedCostUsd ?? null,
        rate_snapshot:      stats.rateSnapshot || null,
        run_id:             stats.runId ?? null,
      });

    if (error) logger.warn(`saveApiUsage error (userId: ${userId}, provider: ${provider}): ${error.message}`);
  } catch (error) {
    logger.warn(`saveApiUsage error (userId: ${userId}, provider: ${provider}): ${error.message}`);
  }
}

export async function saveApiRun(provider, stats, supabase = getSupabaseClient()) {
  try {
    const { data, error } = await supabase
      .from('api_run_logs')
      .insert({
        provider:        provider,
        model_or_actor:  stats.modelOrActor || null,
        source_type:     stats.sourceType || null,
        input_tokens:    stats.inputTokens ?? null,
        output_tokens:   stats.outputTokens ?? null,
        compute_units:   stats.computeUnits ?? null,
        total_items:     stats.totalItems ?? null,
        total_cost_usd:  stats.totalCostUsd ?? null,
        rate_snapshot:   stats.rateSnapshot || null,
      })
      .select('id')
      .single();
    ...
    if (error) {
      logger.warn(`saveApiRun error (provider: ${provider}): ${error.message}`);
      return null;
    }
    return data.id;
  } catch (error) {
...
export async function saveCronExecution(stats, supabase = getSupabaseClient()) {
  try {
    const { data, error } = await supabase
      .from('cron_execution_logs')
      .insert({
        hour_utc:        stats.hourUtc,
        status:          stats.status,
        users_processed: stats.usersProcessed ?? 0,
        posts_sent:      stats.postsSent ?? 0,
        posts_failed:    stats.postsFailed ?? 0,
    ...
    if (error) {
      logger.warn(`saveCronExecution error (hourUtc: ${stats.hourUtc}): ${error.message}`);
      return null;
    }
    return data.id;
  } catch (error) {
...
export async function getRecentCronExecutions(limit = 30, supabase = getSupabaseClient()) {
  try {
    const { data, error } = await supabase
      .from('cron_execution_logs')
      .select('*')
    ...
    if (error) {
      logger.warn(`getRecentCronExecutions error: ${error.message}`);
      return [];
    }
    return data;
  } catch (error) {
...
export async function getApiCostSummary(userId, from, to, supabase = getSupabaseClient()) {
  const { data, error } = await supabase
    .from('api_usage_logs')
  ...
  const summary = { claude: 0, apify: 0 };
  for (const row of data || []) {
    if (row.provider in summary) {
      summary[row.provider] += row.estimated_cost_usd || 0;
    }
  }

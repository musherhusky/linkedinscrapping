...
export async function checkUrlLimit(userId) {
  ...
  // Corporate o plan sin límite configurado: ilimitado
  if (maxUrls === null || maxUrls === undefined) return { allowed: true, current: null, max: null };
...
const VALID_POSTED_LIMITS = new Set(['1h', '24h', '7d', '30d', 'all']);
...
export function validateSettings(settings) {
  ...
  if (settings.posted_limit !== undefined && !VALID_POSTED_LIMITS.has(settings.posted_limit)) {
    throw new Error(`posted_limit inválido: "${settings.posted_limit}". Valores permitidos: ${[...VALID_POSTED_LIMITS].join(', ')}`);
  }

  if (settings.max_posts_per_company !== undefined) {
    const v = settings.max_posts_per_company;
    if (!Number.isInteger(v) || v < 0) {
      throw new Error(`max_posts_per_company debe ser un entero >= 0 (0 = sin límite), recibido: ${v}`);
    }
  }

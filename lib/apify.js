...
async function waitForActorCompletion(actorId, runId, token) {
  ...
  let attempts = 0;
  const maxAttempts = 120; // 120 intentos = 30 minutos (15 seg cada uno)
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      // Esperar 15 segundos antes de siguiente intento
      await delay(15000);
    } catch (error) {
  ...
  throw new Error(`Actor timeout (>30 minutos)`);
}

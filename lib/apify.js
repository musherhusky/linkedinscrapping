...
export async function executeActor(targetUrls, settings) {
  ...
  try {
    ...
    return items.map(item => ({
      url: item.linkedinUrl,                    // ← CORRECTO
      title: item.content?.substring(0, 200) || '',  // Primeros 200 caracteres
      description: item.content || '',          // Contenido completo
      publishedDate: item.postedAt?.date || new Date().toISOString(),  // ← CORRECTO
    }));
  } catch (error) {

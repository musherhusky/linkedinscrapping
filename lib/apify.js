...
function buildActorInput(targetUrls, settings) {
  ...
  // };
  return {
    profileUrls: targetUrls, // Cambio: profileUrls en lugar de targetUrls
    maxPostsPerProfile: settings.max_posts_per_company || 5,
...
async function waitForActorCompletion(actorId, runId, token) {
  ...
  let attempts = 0;
  const maxAttempts = 120; // 120 * 30 = 60 minutos
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      await delay(30000); // Esperar 30 segundos entre intentos
    } catch (error) {
...
async function getActorDataset(actorId, runId, token) {
  ...
  // Esperar 5 segundos extra antes de intentar obtener dataset
  await delay(5000);

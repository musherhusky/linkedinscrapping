...
function buildActorInput(targetUrls, settings) {
  ...
  // LOG del input que estamos enviando
  logger.info(`INPUT enviado a Apify:`, JSON.stringify(input, null, 2));
...
export async function executeActor(targetUrls, settings) {
  ...
  try {
    ...
    // LOG: Ver estructura exacta
    logger.debug(`Response type:`, typeof responseData);
    logger.debug(`Response keys:`, Object.keys(responseData));
    logger.debug(`Full response:`, JSON.stringify(responseData).substring(0, 500));

    // Manejar tanto formato directo como en objeto

...
async function runActor(actorId, token, input) {
  ...
  const runStats = {
    actorId,
    computeUnits: waitData.data.stats?.computeUnits ?? null,
    usageTotalUsd: waitData.data.usageTotalUsd ?? null,
  };
...
export async function executeActor(targetUrls, settings) {
  ...
  try {
    const { items, runStats } = await runActor(actorId, token, input);
...
export async function executeTermsActor(terms, settings) {

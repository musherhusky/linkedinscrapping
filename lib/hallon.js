...
export async function processAndSendToHallon(posts, userId, settings, sourceType = 'company') {
  let sent = 0;
  let failed = 0;
  ...
  for (const post of posts) {
    try {
      ...
      await savePost(userId, post, 'sent', hallonResponse, sourceType);
...
export async function processWithoutHallon(posts, userId, sourceType = 'company') {
  let saved = 0;
  let failed = 0;
  ...
  for (const post of posts) {
    try {
      await savePost(userId, post, 'extracted', null, sourceType);

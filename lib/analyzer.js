    ...
    try {
      ...
      const urlSpecificTopics = userTopics
        ?.filter(t => {
          ...
          return batchAuthorIds.includes(slug);
        })
        .map(t => t.topic) || [];
      ...
      // Llamar a Claude
      const results = await analyzeBatch(batch, forcedTopics, userId);
...
async function saveAnalysisResults(userId, results) {
  ...
  for (const result of results) {
    ...
    for (const category of result.categories || []) {
      const display = category?.display?.trim();
      const canonical = category?.canonical?.trim() || display;
      if (display) {
        categoriesToInsert.push({ post_id: postId, user_id: userId, display, canonical });
      }
    }

    // Temas libres
    for (const topic of result.topics || []) {
      const display = topic?.display?.trim();
      const canonical = topic?.canonical?.trim() || display;
      if (display) {
        topicsToInsert.push({ post_id: postId, user_id: userId, display, canonical, forced: false, confidence: 'high' });
      }
    }
    ...
    for (const ft of result.forced_topics || []) {
      if (ft.mentioned) {
        topicsToInsert.push({ post_id: postId, user_id: userId, display: ft.topic, canonical: ft.topic, forced: true, confidence: ft.confidence || 'low' });
      }
    }
  }

...
import { getActiveCompanies, getActivePeople, getAllUsersForHour, deduplicatePosts, getTodayStats, upsertTargetProfile, insertFollowerHistory, upsertDiscoveredProfile, upsertDiscoveredProfileRelation } from './database.js';
import { executeActor, executePeopleActor, mapProfileEnrichment, mapDiscoveredProfiles } from './apify.js';
...
export async function processAllUsersBatched(hourUtc) {
  ...
  if (apifyEnabledUser) {
    ...
    [companyPostsAll, peoplePostsAll] = await Promise.all([
      allCompanyUrls.length > 0 ? executeActor(allCompanyUrls, refSettings) : Promise.resolve([]),
      allPeopleUrls.length > 0  ? executePeopleActor(allPeopleUrls, refSettings) : Promise.resolve([]),
    ]);
    logger.success(`Empresas: ${companyPostsAll.length} posts | Personas: ${peoplePostsAll.length} posts`);

    const scrapedAt = new Date().toISOString();
    await enrichProfilesFromBatch({ usersData, companyPostsAll, peoplePostsAll, scrapedAt });
  }
  ...
  const totalSent   = results.reduce((s, r) => s + (r.companies?.sent || 0) + (r.people?.sent || 0), 0);
  const totalFailed = results.reduce((s, r) => s + (r.companies?.failed || 0) + (r.people?.failed || 0), 0);
...
async function distributeAndProcess({ userId, settings, plan, companies, people, companyPostsAll, peoplePostsAll }) {
  ...
  try {
    ...
    logger.info(`Usuario ${userId} — empresas: ${userCompanyPosts.length} posts (${companies.length} URLs), personas: ${userPeoplePosts.length} posts (${people.length} URLs)`);
    ...
    logger.success(`Usuario ${userId} completado en ${elapsed}s — enviados: ${(companiesResult.sent || 0) + (peopleResult.sent || 0)}, fallidos: ${(companiesResult.failed || 0) + (peopleResult.failed || 0)}, duplicados: ${(companiesResult.duplicates || 0) + (peopleResult.duplicates || 0)}`);
...
async function enrichProfilesFromBatch({ usersData, companyPostsAll, peoplePostsAll, scrapedAt }) {
  ...
  await Promise.all(
    enrichments.map(async (enrichment) => {
      ...
      await Promise.all(
        userIds.map(async (userId) => {
          try {
            await upsertTargetProfile(userId, enrichment);
            await insertFollowerHistory(userId, enrichment.queryTargetUrl, enrichment.followersCount, scrapedAt);
          } catch (err) {
  ...
  for (let i = 0; i < allDiscovered.length; i += BATCH) {
    ...
    await Promise.all(
      batch.map(async (profile) => {
        ...
        const id = await upsertDiscoveredProfile(profile);
        if (id) await upsertDiscoveredProfileRelation(id, profile.sourceUrl, profile.source);
      })
    );
  }

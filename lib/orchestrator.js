...
async function distributeAndProcess({ userId, settings, plan, companies, people, companyPostsAll, peoplePostsAll }) {
  try {
    ...
    const userCompanyPosts = companyPostsAll.filter(p => companySet.has(p.linkedinUrl) || companySet.has(p.url));
    const userPeoplePosts  = peoplePostsAll.filter(p  => peopleSet.has(p.linkedinUrl)  || peopleSet.has(p.url));

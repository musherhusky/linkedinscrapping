...
async function distributeAndProcess({ userId, settings, plan, companies, people, companyPostsAll, peoplePostsAll }) {
  try {
    ...
    const userCompanyPosts = companyPostsAll.filter(p => companySet.has(p.query?.targetUrl) || companySet.has(p.author?.linkedinUrl));
    const userPeoplePosts  = peoplePostsAll.filter(p  => peopleSet.has(p.query?.targetUrl)  || peopleSet.has(p.author?.linkedinUrl));

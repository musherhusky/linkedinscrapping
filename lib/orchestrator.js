...
import { getActiveCompanies, getActivePeople, getActiveTerms, getAllUsersForHour, deduplicatePosts, getTodayStats, upsertTargetProfile, insertFollowerHistory, upsertDiscoveredProfile, upsertDiscoveredProfileRelation, saveApiUsage } from './database.js';
...
export async function processAllUsersBatched(hourUtc, deps = {}) {
  ...
  // 3. Deduplicar URLs/términos globalmente
...
async function distributeAndProcess({
  ...
  try {
    ...
    await Promise.all([
      logApifyUsageShare(userId, 'company', companyRunStats, userCompanyPosts.length, companyPostsAll.length, deps),
      logApifyUsageShare(userId, 'person', peopleRunStats, userPeoplePosts.length, peoplePostsAll.length, deps),
      logApifyUsageShare(userId, 'term', termsRunStats, userTermPosts.length, (termPostsAll || []).length, deps),
    ]);
...
// (see design.md Decision 5 in openspec/changes/track-api-costs).
async function logApifyUsageShare(userId, sourceType, runStats, userPostsCount, totalPostsCount, deps = {}) {
...
export async function processUser(userId, deps = {}) {
  const {
    executeActor: runCompanyActor = executeActor,
    saveApiUsage: logUsage = saveApiUsage,
  } = deps;
    ...
    if (runStats) {
      try {
        await logUsage(userId, 'apify', {
          ...
          rateSnapshot: { source: 'apify_usageTotalUsd', sourceType: 'company' },
        });
      } catch (error) {
        logger.warn(`No se pudo registrar el uso de Apify: ${error.message}`);
      }
    }
    ...
    if (runStats) {
      try {
        await logUsage(userId, 'apify', {
          ...
          postsReceived: posts.length,
          estimatedCostUsd: runStats.usageTotalUsd,
          rateSnapshot: { source: 'apify_usageTotalUsd', sourceType: 'person' },
        });
      } catch (error) {
        logger.warn(`No se pudo registrar el uso de Apify: ${error.message}`);
      }
    ...
    if (runStats) {
      try {
        await logUsage(userId, 'apify', {
          ...
          postsReceived: posts.length,
          estimatedCostUsd: runStats.usageTotalUsd,
          rateSnapshot: { source: 'apify_usageTotalUsd', sourceType: 'term' },
        });
      } catch (error) {
        logger.warn(`No se pudo registrar el uso de Apify: ${error.message}`);
      }

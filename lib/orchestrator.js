...
import { getActiveCompanies, getActivePeople, getActiveTerms, getAllUsersForHour, deduplicatePosts, getTodayStats, upsertTargetProfile, insertFollowerHistory, upsertDiscoveredProfile, upsertDiscoveredProfileRelation } from './database.js';
...
export async function processAllUsersBatched(hourUtc, deps = {}) {
  ...
  if (apifyEnabledUser) {
    ...
    [companyPostsAll, peoplePostsAll, termPostsAll] = await Promise.all([
      allCompanyUrls.length > 0 ? runCompanyActor(allCompanyUrls, refSettings) : Promise.resolve([]),
      allPeopleUrls.length > 0  ? runPeopleActor(allPeopleUrls, refSettings) : Promise.resolve([]),
      allTerms.length > 0       ? runTermsActor(allTerms, refSettings) : Promise.resolve([]),
    ]);
  ...
  const results = await Promise.all(
    usersData.map(({ userId, settings, plan, companies, people, terms }) =>
      distributeAndProcess({ userId, settings, plan, companies, people, terms, companyPostsAll, peoplePostsAll, termPostsAll }, deps)
    )
  );
...
async function distributeAndProcess({ userId, settings, plan, companies, people, terms, companyPostsAll, peoplePostsAll, termPostsAll }, deps = {}) {
...
export async function processTerms(userId, deps = {}) {
  ...
  try {
    ...
    const posts = await runTermsActor(terms, { ...settings, posted_limit: plan.plans.posted_limit });

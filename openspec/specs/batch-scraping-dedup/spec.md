# Capability: Batch Scraping Dedup

## Purpose

Ensure the batched cron path queries Apify once per unique company/person target across all users sharing an hour, regardless of superficial URL formatting differences (trailing slash, casing) between different users' registrations — preventing duplicate Apify spend and duplicate post delivery/dispatch.

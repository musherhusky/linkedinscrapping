...
Users can currently track LinkedIn companies and people, and every scrape run pulls posts for those tracked URLs via Apify. A new `target_search_terms` table lets users track free-text search terms instead of profile URLs, but nothing in the scraping pipeline reads it yet. Users need term-based monitoring (e.g. tracking a keyword or hashtag across LinkedIn) without waiting for a manually-triggered profile URL to exist for every topic they care about.

## What Changes

- Add `getActiveTerms(userId)` to `lib/database.js`, reading active rows from `target_search_terms` (mirrors `getActiveCompanies` / `getActivePeople`).
...
- **Database**: new `target_search_terms` table (already created by the user); `posts.source_type` gains a third valid value, `'term'`.
